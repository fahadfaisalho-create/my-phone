...
export class MailService {
  ...
  async sendStoreApproval(to: string, storeName: string) {
    await this.send(
      ...
      `تهانينا! تم قبول طلب تسجيل محلك "${storeName}" على منصة صيانة وبيع الجوالات. يمكنك الآن تسجيل الدخول وإكمال إعداد محلك.`,
    );
  }
  ...
  async sendPasswordResetToken(to: string, token: string) {
    await this.send(
      to,
      'استعادة كلمة السر — منصة صيانة وبيع الجوالات',
