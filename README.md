...
أربع واجهات تشترك بنفس الـ API (`apps/api`) وقاعدة بيانات واحدة:

1. **تسجيل المحل (لوحة التاجر)** — صاحب محل (شركة) أو فني مستقل يسجّل ببياناته + السجل التجاري/الهوية وتصديق الحساب البنكي. الحساب يدخل بحالة `pending` (قيد المراجعة) ولا يظهر للمستهلكين بعد.
2. **مراجعة الإدارة (لوحة الإدارة)** — الأدمن يشوف طلبات التسجيل، يقبل أو يرفض (مع سبب). بعد القبول تصير حالة المحل `active` ويظهر بتطبيق المستهلك فوراً.
3. **تجهيز المحل (لوحة التاجر)** — بعد التفعيل، التاجر يضيف فروعه، فريق الصيانة، الخدمات، والمنتجات مع صور وأسعار ومخزون. الأقسام تُفتح تدريجياً (مثلاً: المنتجات ما تُفتح إلا بعد إضافة فرع واحد على الأقل) لضمان محل مكتمل البيانات قبل ما يظهر للمستهلك.
4. **التصفح والتفاعل (تطبيق المستهلك)** — المستهلك يسجّل دخول برقم جواله (رمز تحقق OTP)، يتصفح المحلات، يشوف تفاصيل محل معيّن (فروع/خدمات/منتجات/فريق الصيانة/تقييمات)، ثم يحجز موعد صيانة أو يشتري منتج (سلة + محاكاة دفع).
5. **التواصل والمتابعة** — شات فوري (Socket.io) بين المستهلك والمحل، وتذاكر دعم لأي طرف (مستهلك أو تاجر) ترفع للإدارة.

باختصار: **تسجيل ← مراجعة ← تجهيز ← تصفّح ← حجز/شراء ← تواصل** — كل خطوة محمية بصلاحية دور مناسب (`admin` / `merchant_rep` / `consumer`) ومُختبرة فعلياً ضد قاعدة بيانات حقيقية من أول خطوة لآخرها.
