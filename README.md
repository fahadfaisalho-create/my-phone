...
- ✅ تطبيق المستهلك (`apps/mobile-consumer`، React Native/Expo) — **نسخة أولى** حسب خطة المواصفات: تسجيل دخول OTP، تصفح المحلات النشطة (بحث)، عرض ملف محل (خدمات/منتجات/تقييمات) وإرسال تقييم. الشات والحجز والشراء الفعلي مؤجلة لمراحل لاحقة (الـ API لها جاهز فعلاً، الواجهة لم تُبنَ بعد بالتطبيق عمداً حسب ترتيب خطة المواصفات).
...
## الخطوة التالية
حسب خطة المواصفات: الشات المباشر (Socket.io) ثم الحجز والشراء والدفع الفعلي — الـ API لهذه الوظائف جاهز، تبقى واجهات تطبيق المستهلك.
